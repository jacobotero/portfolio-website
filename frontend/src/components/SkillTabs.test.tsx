...
describe('SkillTabs', () => {
  ...
  it('selects the first category by default', () => {
    render(<SkillTabs />)
    expect(screen.getByRole('tab', { name: SKILL_GROUPS[0].label })).toHaveAttribute(
      'aria-selected',
      'true',
    )
  })
  ...
  it('switches the visible skills when another tab is chosen', async () => {
    ...
    await user.click(screen.getByRole('tab', { name: SKILL_GROUPS[1].label }))
    ...
    expect(screen.getByRole('tab', { name: SKILL_GROUPS[1].label })).toHaveAttribute(
      'aria-selected',
      'true',
    )
  })
